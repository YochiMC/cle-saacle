...
Artisan::command('qualifications:import {file : Ruta del archivo .xlsx/.csv} {--disk= : Disco de Storage opcional}', function (): int {
    $file = (string) $this->argument('file');
    $disk = $this->option('disk');
    $import = new QualificationsImport();
    ...
    $this->info('Importación de calificaciones finalizada.');
    $this->line('Filas procesadas: '.$import->getProcessedRows());
    $this->line('Filas importadas: '.$import->getImportedRows());
    $this->line('Duplicados omitidos (student + level + period): '.$import->getSkippedDuplicates());
    ...
    return self::SUCCESS;
})->purpose('Importa calificaciones de legado desde un Excel/CSV con encabezados');
