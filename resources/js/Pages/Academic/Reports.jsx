 ...
 */
const GROUP_TYPE_VALUES = ["Regular", "Intensivo", "Semi intensivo", "Programa Especial"];
const EXAM_TYPE_VALUES  = ["Convalidación", "Planes anteriores", "4 habilidades", "Ubicación"];

/** Helpers para normalizar valores de enum que pueden llegar como objeto o string */
const resolveEnum = (val) => (typeof val === "object" && val !== null ? val.value ?? val : val);
...
export default function Reports({
    ...
    const [charts, setCharts] = useState([
        { id: 1, type: "estatus", filterType: "todos", filterPeriod: "", filterCourseType: "todos", filterSpecificType: "", filterEntity: "", customTitle: "", description: "" },
        { id: 2, type: "tasa_aprobacion", filterType: "todos", filterPeriod: "", filterCourseType: "todos", filterSpecificType: "", filterEntity: "", customTitle: "", description: "" },
    ]);
    ...
    const getSpecificTypeOptions = (courseType) => {
        ...
        if (courseType === "todos" || courseType === "curso") {
            GROUP_TYPE_VALUES.forEach(v => opts.push({ value: v, label: `Curso ${v}` }));
        }
        if (courseType === "todos" || courseType === "examen") {
            EXAM_TYPE_VALUES.forEach(v => opts.push({ value: v, label: `Examen ${v}` }));
        }
        return opts;
    };
    ...
    const getAvailableEntities = (config) => {
        ...
        if (config.filterCourseType !== "examen") {
            groups
                .filter(g => !config.filterPeriod || String(g.period_id) === String(config.filterPeriod))
                .filter(g => !config.filterSpecificType || resolveEnum(g.type) === config.filterSpecificType)
                .forEach(g => available.push({ value: `g_${g.id}`, label: `(Grupo) ${g.name}` }));
        }

        if (config.filterCourseType !== "curso") {
            exams
                .filter(e => !config.filterPeriod || String(e.period_id) === String(config.filterPeriod))
                .filter(e => !config.filterSpecificType || resolveEnum(e.exam_type) === config.filterSpecificType)
                .forEach(e => available.push({ value: `e_${e.id}`, label: `(Examen) ${e.name}` }));
        }
    ...
    const getChartData = (cfg) => {
        ...
        if (cfg.filterType === "vigentes") {
            filtered = filtered.filter(s => resolveEnum(s.type_student) === "vigente");
        } else if (cfg.filterType === "egresados") {
            filtered = filtered.filter(s => resolveEnum(s.type_student) === "egresado");
        }

        // ── 2. Filtro de periodo ──
        if (cfg.filterPeriod) {
            filtered = filtered.filter(s =>
                s.period_ids && s.period_ids.some(id => String(id) === String(cfg.filterPeriod))
            );
        }
        ...
        if (cfg.filterCourseType === "curso") {
            if (cfg.filterPeriod) {
                filtered = filtered.filter(s =>
                    s.course_period_ids && s.course_period_ids.some(id => String(id) === String(cfg.filterPeriod))
                );
            } else {
                filtered = filtered.filter(s => s.course_period_ids && s.course_period_ids.length > 0);
            }
        } else if (cfg.filterCourseType === "examen") {
            if (cfg.filterPeriod) {
                filtered = filtered.filter(s =>
                    s.exam_period_ids && s.exam_period_ids.some(id => String(id) === String(cfg.filterPeriod))
                );
            } else {
                filtered = filtered.filter(s => s.exam_period_ids && s.exam_period_ids.length > 0);
            }
        }

        // ── 4. Filtro de tipo específico (ej. "Regular", "Convalidación") ──
        if (cfg.filterSpecificType) {
            const isGroupType = GROUP_TYPE_VALUES.includes(cfg.filterSpecificType);
            const isExamType  = EXAM_TYPE_VALUES.includes(cfg.filterSpecificType);

            if (isGroupType) {
                const matchIds = new Set();
                groups
                    .filter(g => resolveEnum(g.type) === cfg.filterSpecificType)
                    .filter(g => !cfg.filterPeriod || String(g.period_id) === String(cfg.filterPeriod))
                    .forEach(g => (g.qualifications || []).forEach(q => matchIds.add(q.student_id)));
                filtered = filtered.filter(s => matchIds.has(s.id));
            } else if (isExamType) {
                const matchIds = new Set();
                exams
                    .filter(e => resolveEnum(e.exam_type) === cfg.filterSpecificType)
                    .filter(e => !cfg.filterPeriod || String(e.period_id) === String(cfg.filterPeriod))
                    .forEach(e => (e.students || []).forEach(s => matchIds.add(s.id)));
                filtered = filtered.filter(s => matchIds.has(s.id));
            }
        }
        ...
        switch (cfg.type) {
            ...
            case "tasa_aprobacion": {
                ...
                if (cfg.filterEntity?.startsWith("g_")) {
                    const group = groups.find(g => String(g.id) === cfg.filterEntity.replace("g_", ""));
                    (group?.qualifications || []).forEach(q => {
                        if (validIds.has(q.student_id)) {
                            parseFloat(q.final_average) >= 70 ? aprobados++ : reprobados++;
                        }
                    });
                } else if (cfg.filterEntity?.startsWith("e_")) {
                    const exam = exams.find(e => String(e.id) === cfg.filterEntity.replace("e_", ""));
                    (exam?.students || []).forEach(s => {
                        if (validIds.has(s.id) && s.pivot) {
                            parseFloat(s.pivot.final_average || s.pivot.calificacion || 0) >= 70 ? aprobados++ : reprobados++;
                        }
                    });
                } else {
                    ...
                    if (cfg.filterCourseType !== "examen") {
                        groups
                            .filter(g => !cfg.filterPeriod || String(g.period_id) === String(cfg.filterPeriod))
                            .filter(g => !cfg.filterSpecificType || resolveEnum(g.type) === cfg.filterSpecificType)
                            .forEach(g => (g.qualifications || []).forEach(q => {
                                if (validIds.has(q.student_id)) {
                                    parseFloat(q.final_average) >= 70 ? aprobados++ : reprobados++;
                                }
                            }));
                    ...
                    if (cfg.filterCourseType !== "curso") {
                        exams
                            .filter(e => !cfg.filterPeriod || String(e.period_id) === String(cfg.filterPeriod))
                            .filter(e => !cfg.filterSpecificType || resolveEnum(e.exam_type) === cfg.filterSpecificType)
                            .forEach(e => (e.students || []).forEach(s => {
                                if (validIds.has(s.id) && s.pivot) {
                                    parseFloat(s.pivot.final_average || s.pivot.calificacion || 0) >= 70 ? aprobados++ : reprobados++;
                                }
                            }));
            ...
            case "tipo_curso": {
                return GROUP_TYPE_VALUES.map(tipo => {
                    ...
                    groups
                        ...
                        .filter(g => !cfg.filterPeriod || String(g.period_id) === String(cfg.filterPeriod))
                        .forEach(g => (g.qualifications || []).forEach(q => {
                            if (validIds.has(q.student_id)) studentIds.add(q.student_id);
                        }));
            ...
            case "por_grupo": {
                return groups
                    ...
                    .map(g => ({
                        name: g.name,
                        total: (g.qualifications || []).filter(q => validIds.has(q.student_id)).length,
                    }))
                    .filter(d => d.total > 0);
            }
            ...
            case "por_examen": {
                return exams
                    ...
                    .map(e => ({
                        name: e.name,
                        total: (e.students || []).filter(s => validIds.has(s.id)).length,
                    }))
                    .filter(d => d.total > 0);
            }
            ...
            case "cursos_vs_examenes": {
                ...
                let examenes = 0;
                filtered.forEach(s => {
                    if (s.course_period_ids?.some(id => cfg.filterPeriod ? String(id) === String(cfg.filterPeriod) : true)) cursos++;
                    if (s.exam_period_ids?.some(id => cfg.filterPeriod ? String(id) === String(cfg.filterPeriod) : true)) examenes++;
                });
                return [
            ...
            case "periodo":
                return periods.map(p => ({
                    name: p.name,
                    total: filtered.filter(s => s.period_ids?.some(id => String(id) === String(p.id))).length,
                }));
    ...
    const getChartTitle = (cfg) => {
        ...
        return (metricas[cfg.type] || "Gráfica") + (poblacion[cfg.filterType] || "");
    };
            ...
            <div className="min-h-screen bg-gray-100 py-12 print:bg-white print:py-0">
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-6 px-8 py-5 print:shadow-none print:border-0 print:mb-4 print:px-0">
                            <div className="flex items-center justify-between gap-6">
                                <div className="flex items-center justify-start flex-1">
                                    {logos.logo_sep ? (
                                        <img src={logos.logo_sep} alt="Secretaría de Educación Pública" className="h-14 object-contain" />
                                    ) : <div className="h-14 w-24 bg-gray-100 rounded animate-pulse" />}
                                </div>
                                <div className="flex flex-col items-center text-center">
                                    {logos.logo_tecnm ? (
                                        <img src={logos.logo_tecnm} alt="Tecnológico Nacional de México" className="h-16 object-contain mb-1" />
                                ...
                                <div className="flex items-center justify-end flex-1">
                                    {logos.logo_itl ? (
                                        <img src={logos.logo_itl} alt="Instituto Tecnológico de Lázaro Cárdenas" className="h-14 object-contain" />
                                    ) : <div className="h-14 w-24 bg-gray-100 rounded animate-pulse" />}
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 print:grid-cols-1 gap-6">
                        {charts.map((cfg) => (
                            <div key={cfg.id} className="bg-white rounded-lg shadow flex flex-col print:shadow-none print:break-inside-avoid print:mb-8">

                                {/* CONTROLES DE CONFIGURACIÓN */}
                                <div className="p-4 border-b border-gray-200 bg-gray-50 flex flex-col gap-3 rounded-t-lg print:hidden">
                                    <div className="flex justify-between items-center">
                                        <h3 className="font-semibold text-gray-700">Configuración del Gráfico</h3>
                                        <button
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                                        ...
                                        <div>
                                            ...
                                            <select
                                                value={cfg.type}
                                                onChange={(e) => updateChart(cfg.id, "type", e.target.value)}
                                            ...
                                            >
                                                <optgroup label="Demografía">
                                                    <option value="estatus">Estatus del Alumno</option>
                                                ...
                                                <optgroup label="Rendimiento Académico">
                                                    <option value="tasa_aprobacion">Tasa de Aprobación</option>
                                                    <option value="cursos_vs_examenes">Cursos vs Exámenes</option>
                                                </optgroup>
                                                <optgroup label="Desglose por Tipo (Enums)">
                                                    <option value="tipo_curso">Alumnos por Tipo de Curso</option>
                                                    <option value="tipo_examen">Alumnos por Tipo de Examen</option>
                                                    <option value="por_grupo">Alumnos por Grupo Individual</option>
                                                    <option value="por_examen">Alumnos por Examen Individual</option>
                                                </optgroup>
                                        ...
                                        <div>
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => updateChart(cfg.id, "filterType", e.target.value)}
                                                className="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                            >
                                        ...
                                        <div>
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => updateChart(cfg.id, "filterPeriod", e.target.value)}
                                                className="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                            >
                                        ...
                                        <div>
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => updateChart(cfg.id, "filterCourseType", e.target.value)}
                                                className="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                            >
                                        ...
                                        <div>
                                            <label className="block text-xs font-medium text-gray-600 mb-1">Tipo Específico</label>
                                            <select
                                                value={cfg.filterSpecificType}
                                                onChange={(e) => updateChart(cfg.id, "filterSpecificType", e.target.value)}
                                                className="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                            >
                                                <option value="">Todos los tipos</option>
                                                {getSpecificTypeOptions(cfg.filterCourseType).map(opt => (
                                        ...
                                        {cfg.type === "tasa_aprobacion" && (
                                            <div>
                                                <label className="block text-xs font-medium text-gray-600 mb-1">Grupo/Examen</label>
                                                <select
                                                    value={cfg.filterEntity}
                                                    onChange={(e) => updateChart(cfg.id, "filterEntity", e.target.value)}
                                                    className="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                                >
                                                    <option value="">Combinado global</option>
                                                    {getAvailableEntities(cfg).map(ent => (
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
                                        <div>
                                            <label className="block text-xs font-medium text-gray-600 mb-1">Título Personalizado</label>
                                            <input
                                                type="text"
                                                value={cfg.customTitle}
                                                onChange={(e) => updateChart(cfg.id, "customTitle", e.target.value)}
                                                placeholder={getChartTitle(cfg)}
                                                className="w-full text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-gray-600 mb-1">Descripción para impresión</label>
                                            <input
                                                type="text"
                                                value={cfg.description}
                                                onChange={(e) => updateChart(cfg.id, "description", e.target.value)}
                                ...
                                <div className="p-6 flex-1 flex flex-col justify-center print:p-2">
                                    <Graficas
                                        title={cfg.customTitle || getChartTitle(cfg)}
