                    ...
                    <div className="overflow-hidden bg-white border border-gray-100 shadow-sm sm:rounded-2xl group">
                        ...
                        <div className="divide-y divide-gray-100">
                            {displayedServices.length > 0 ? displayedServices.map((pago) => (
                                <div key={pago.id} className="flex items-center justify-between gap-4 p-4 hover:bg-gray-50 rounded-lg cursor-pointer" onClick={() => handleOpenPayment(pago)}>
                                    <div className="flex items-center gap-3">
                                        <div className={`p-3 rounded-md flex-shrink-0 border ${pago.status === 'approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-white text-indigo-600 border-indigo-100'}`}>
                                            {pago.status === 'approved' ? <CheckCircle className="w-5 h-5" /> : <CreditCard className="w-5 h-5" />}
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <h4 className="font-semibold text-gray-900">{getTypeLabel(pago.type)}</h4>
                                                <span className="text-sm text-gray-500">{new Date(pago.created_at).toLocaleDateString()}</span>
                                            </div>
                                            {isAdmin && pago.student && <p className="text-sm text-gray-500">{pago.student.user?.name}</p>}
                                        </div>
                                    </div>

                                    <div className="flex items-center gap-3">
                                        <div className="text-right">
                                            <div className="text-lg font-bold text-gray-900">{formatCurrency(pago.amount)}</div>
                                            <div className="mt-1">
                                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${getStatusStyle(pago.status)}`}>
                                ...
                                <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
                                    ...
                                    <h3 className="mb-2 text-lg font-semibold text-gray-900">No hay pagos registrados</h3>
                                    <p className="max-w-sm mx-auto leading-relaxed text-gray-500">Sube tu comprobante para liberar la inscripción.</p>
                                </div>
                            )}
                        </div>
                    </div>
